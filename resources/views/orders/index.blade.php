...
@section('content')
<section class="admin-order-index">
        ...
        <div class="section-container shadow-lg sm:rounded-lg overflow-hidden relative">
            ...
            <div class="header-section p-6 flex flex-col md:flex-row items-center justify-between">
                ...
                <div class="search-container w-full md:w-1/2 flex items-center">
                    <form class="flex w-full" action="{{ route('orders.index') }}" method="GET">
                    ...
                    @if(request('search'))
                    <div class="ml-2">
                        <a href="{{ route('orders.index') }}" class="back-btn add-button px-4 py-2 hover:text-brown-900 font-medium rounded-lg text-sm">
                            Back to All List
                        </a>
                    </div>
                    @endif
            ...
            <!-- Page Title -->
            <h1 class="page-title">Laundry Orders</h1>
                ...
                <table class="customer-table">
                    ...
                    <tbody>
                        @foreach ($orders as $order)
                            <tr class="table-row">
                                <td class="table-data">{{ $order->customer ? $order->customer->name : 'No Customer' }}</td>
                                <td class="table-data">{{ $order->service->service_name }}</td>
                                <td class="table-data">{{ \Carbon\Carbon::parse($order->order_date)->format('d M Y') }}</td>
                                <td class="table-data">{{ $order->status }}</td>
                                <td class="table-data">{{ $order->weight }} kg</td>
                                <td class="table-data">Rp {{ number_format($order->service->price * $order->weight) }}</td>
                                <td class="table-data">{{ $order->date_taken ? \Carbon\Carbon::parse($order->date_taken)->format('d M Y') : '-' }}</td>
...
<script>
    let orderToDelete = null; // Variable to hold the order ID for deletion
    function openDeleteModal(orderId) {
        orderToDelete = orderId; // Store the order ID
        document.getElementById('deleteConfirmationModal').style.display = 'flex'; // Show modal
    }

    function confirmDelete() {
        document.querySelector(`form[action$="orders/${orderToDelete}"]`).submit(); // Submit the delete form
    }
</script>
...
<div id="deleteConfirmationModal" class="modal" style="display: none;">
    <div class="modal-content">
        ...
        <div class="modal-buttons">
            <button id="confirmDeleteBtn" onclick="confirmDelete()">Yes, Delete Order</button>
